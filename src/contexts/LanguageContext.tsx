...
const translations = {
  ...
  ja: {
    ...
    "dashboard.step3Point2": "為替レートの影響を除き、元本は減少しません",
    ...
    "riskMitigation.risk1.description":
      "あなたの資金は自動化された公開ソフトウェア（スマートコントラクト）によって管理されます。セキュリティの近年の大幅な改善と我々のセキュリティに対する努力にも関わらず、コードの欠陥がハッカーによって悪用され、投資元本を失う可能性があります。",
    ...
    "riskMitigation.risk1.mitigation1.text":
      "四半期ごとに高水準のセキュリティ監査に投資し、安全なプログラミング言語Moveで作成されたブロックチェーンであるSui上の2〜3の別々の最高水準のレンディングプロトコル（規模と安定性で測定）のみで資産を展開します。",
    "riskMitigation.risk1.mitigation2.title": "投資家保護基金（IPF）：",
    "riskMitigation.risk1.mitigation2.text":
      "当社の分離資本に裏付けられた投資家保護基金（IPF）を設立し、ハッキングが発生した場合に損失の20％を即座に補償します。",
    "riskMitigation.risk1.mitigation3.title": "完全回復への取り組み：",
    "riskMitigation.risk1.mitigation3.text":
      "ハッキングが発生した場合、直ちにL1チェーン財団（例：Sui Foundation）とプロトコルのガバナンスコミュニティ（DAO）に働きかけ、法的・技術的な取り組みを全面的に展開し、コミュニティ資金による資産の完全回復を最大化します。過去に振り返りますと、主要なL1チェーンとプロトコルは失われた投資家資本を補償するケースが多くあります。Perkloopはあなたの代弁者として、あらゆる手段で資金回復を確保することを約束します。",
